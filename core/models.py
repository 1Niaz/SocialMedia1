from django.db import models
from django.contrib.auth import get_user_model
import uuid
from datetime import datetime

# Create your models here.

User = get_user_model()

''' Модель профилей '''
class Profile(models.Model):
    user = models.ForeignKey(User, on_delete=models.CASCADE)
    id_user = models.IntegerField()
    bio = models.TextField(blank=True)
    profileimg = models.ImageField(upload_to='profile_images', default='blank-profile-picture.png')
    location = models.CharField(max_length=100, blank=True)

    # user_post = models.ForeignKey(Post, on_delete=models.CASCADE)

    def __str__(self):
        return f'{self.user.username}'

    verbose_name = 'Profile'
    verbose_name_plural = 'Profiles'

''' Модель постов '''
class Post(models.Model):
    id = models.UUIDField(primary_key=True, default=uuid.uuid4)
    user = models.CharField(max_length=100)
    image = models.ImageField(upload_to='post_images')
    caption = models.TextField()
    created_at = models.DateTimeField(default=datetime.now)
    no_of_likes = models.IntegerField(default=0)

    def __str__(self):
        # return self.user
        return f'{self.user}, {self.caption}, {self.created_at}'
    
    class Meta:
        verbose_name = 'Post'
        verbose_name_plural = 'Posts'

class LikePost(models.Model):
    post_id = models.CharField(verbose_name='Post Id', max_length=500)
    username = models.CharField(verbose_name='Username', max_length=100)
    
    def __str__(self):
        return f'{self.username}'
    
    class Meta:
        verbose_name = 'Post Like'
        verbose_name_plural = 'Post Likes'

class FollowersCount(models.Model):
    follower = models.CharField(verbose_name='Follower', max_length=100)
    user = models.CharField(verbose_name='User', max_length=100)

    def __str__(self):
        return f'{self.user}, {self.follower}'
    
    class Meta:
        verbose_name = 'Follower'
        verbose_name_plural = 'Followers'






